...
function YieldCardWidget({
  ...
  }, [1000, 2000, 4000, 10000, 25000].map(amt => React.createElement('button', {
    key: amt,
    type: 'button',
  ...
  React.createElement('div', {
    ...
    className: 'visa-gold-chip',
    'aria-hidden': 'true'
  }, React.createElement('span', {
    className: 'chip-circuit-line'
  })), renderVisaSvg()), React.createElement('div', {
    className: 'visa-card-center'
  }, React.createElement('div', {
    className: 'visa-card-label-sub'
  }, 'DEFI GARDEN • VIRTUAL ISSUING'), React.createElement('div', {
    className: 'visa-card-funded-label'
  }, isKorean ? `${selectedSub.name} ${_t('yieldCard.cardDedicatedSuffix') !== 'yieldCard.cardDedicatedSuffix' && _t('yieldCard.cardDedicatedSuffix') || '결제 전용'}` : `${selectedSub.name.toUpperCase()} ${_t('yieldCard.cardFundedSuffix') !== 'yieldCard.cardFundedSuffix' && _t('yieldCard.cardFundedSuffix') || 'FUNDED'}`)), React.createElement('div', {
    className: 'visa-card-bottom-row'
  }, React.createElement('div', {
    className: 'visa-card-network-info'
  }, `${(pool.chain || 'BASE').toUpperCase()} ${(pool.symbol || 'USDC').toUpperCase()} • ${Number(totalApy || 0).toFixed(1)}% ${_t('yieldCard.liveApyFunded') !== 'yieldCard.liveApyFunded' && _t('yieldCard.liveApyFunded') || 'YIELD FUNDED'}`), React.createElement('div', {
    className: 'visa-card-cap-badge'
  }, isKorean && selectedSub.monthlyCostKrw ? _t('yieldCard.cardCapKrw', _formatNum(selectedSub.monthlyCostKrw)) || `월 한도: ₩${_formatNum(selectedSub.monthlyCostKrw)}` : _t('yieldCard.cardCap', selectedSub.monthlyCostUsd.toFixed(2)) || `CAP: $${selectedSub.monthlyCostUsd.toFixed(2)}/MO`)))),
  ...
  }, _t('yieldCard.receiptTitle') || 'Waitlist Spot Reserved 🌱'), React.createElement('p', {
    className: 'receipt-summary'
  }, `${pool.symbol || 'USDC'} Card • ${selectedSub.name} • ${isKorean && selectedSub.monthlyCostKrw ? `₩${_formatNum(selectedSub.monthlyCostKrw)}/mo` : `$${selectedSub.monthlyCostUsd.toFixed(2)}/mo`}`), React.createElement('p', {
