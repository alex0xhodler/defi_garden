# OHM-FRAXBP — curve-dex on Ethereum

**Total APY:** 0.85% (0.81% Base · + 0.04% Rewards)

**TVL:** $167.3K

**Pool Type:** LP/DEX

**30d Mean APY:** 0.28%

**Exposure:** multi

**IL Risk:** Yes

## Risk Assessment: Medium

Moderate risk profile

- Low liquidity
- Impermanent loss risk

## The long game

$1,000 in this pool grows to ~$1,043 in 5y at current rates.

Your deposit stays yours — you keep your money, and it keeps working.

This pool's rate moves a lot: 0.85% right now vs a 0.28% 30-day average. Reward emissions change daily — projections on this page use the current rate and will move with it.

[Garden this pool → ~$1,043 in 5y](https://www.defi.garden/plan.html?goal=retirement&pace=rwa&capital=1000&fm=capital&years=5&src=pool)

[Start Earning on curve-dex](https://curve.finance/?ref=defi.garden)

Last updated August 18, 2026
