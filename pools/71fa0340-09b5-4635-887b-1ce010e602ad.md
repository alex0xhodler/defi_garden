# MSETH-WETH — stake-dao on Ethereum

**Total APY:** 3.34% (0.24% Base · + 3.10% Rewards)

**TVL:** $6.33M

**Pool Type:** Yield Farming

**30d Mean APY:** 9.90%

**Exposure:** multi

**IL Risk:** No

## Risk Assessment: Low

Conservative DeFi strategy

- Medium liquidity

## The long game

$1,000 in this pool grows to ~$1,178 in 5y at current rates.

Your deposit stays yours — you keep your money, and it keeps working.

This pool's rate moves a lot: 3.34% right now vs a 9.90% 30-day average. Reward emissions change daily — projections on this page use the current rate and will move with it.

[Garden this pool → ~$1,178 in 5y](https://www.defi.garden/plan.html?goal=retirement&pace=stable&capital=1000&fm=capital&years=5&src=pool)

[Start Earning on stake-dao](https://stakedao.org/?ref=defi.garden)

Last updated August 7, 2026
