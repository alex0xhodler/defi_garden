# MSUSD-FRAXBP — curve-dex on Ethereum

**Total APY:** 0.43% (0.04% Base · + 0.39% Rewards)

**TVL:** $1.1M

**Pool Type:** LP/DEX

**30d Mean APY:** 3.77%

**Exposure:** multi

**IL Risk:** No

## Risk Assessment: Medium

Moderate risk profile

- Medium liquidity
- Impermanent loss risk

## The long game

$1,000 in this pool grows to ~$1,022 in 5y at current rates.

Your deposit stays yours — you keep your money, and it keeps working.

This pool's rate moves a lot: 0.43% right now vs a 3.77% 30-day average. Reward emissions change daily — projections on this page use the current rate and will move with it.

[Garden this pool → ~$1,022 in 5y](https://www.defi.garden/plan.html?goal=retirement&pace=rwa&capital=1000&fm=capital&years=5&src=pool)

[Start Earning on curve-dex](https://curve.finance/?ref=defi.garden)

Last updated September 26, 2026
