# SOL-BSOL — orca-dex on Solana

**Total APY:** 1.07% (1.04% Base · + 0.03% Rewards)

**TVL:** $441.1K

**Pool Type:** LP/DEX

**30d Mean APY:** 0.57%

**Exposure:** multi

**IL Risk:** Yes

## Risk Assessment: Medium

Moderate risk profile

- Low liquidity
- Impermanent loss risk

## The Long Game

$1,000 in this pool grows to ~$1,055 in 5y at current rates.

Your deposit stays yours — you keep your money, and it keeps working.

This pool's rate moves a lot: 1.07% right now vs a 0.57% 30-day average. Reward emissions change daily — projections on this page use the current rate and will move with it.

[Garden this pool → ~$1,055 in 5y](https://www.defi.garden/plan.html?goal=retirement&pace=rwa&capital=1000&fm=capital&years=5&src=pool)

[Start Earning on orca-dex](https://www.orca.so/?ref=defi.garden)

Last updated August 4, 2026
