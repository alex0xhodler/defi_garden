...
| 027 | Share-card generator: real bundled service LOGO on the first row (not 🌱) + fix label/headline mismatch (screenshot: "Spotify" label under a "ChatGPT Plus" headline). Hero viral artifact — canvas taint bug breaks all image shares, so NEEDS a browser+network run to build+verify | 8.6 | READY (spec written; browser+network run required — not buildable in a no-network/no-browser session) | HIGH | specs/027.md | 0 | — |
