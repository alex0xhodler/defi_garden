...
if [ -f .git/index.lock ]; then
  if [ -n "$(find .git/index.lock -mmin +10 2>/dev/null)" ]; then
    ...
    echo "git lock is fresh (<10 min) — a loop is mid-commit; rerun this in a few minutes"; exit 1
  fi
fi
...
if ! git diff --cached --quiet; then
  git commit -m "feat: product-loop-kit + approved loop work (001,002,004)"
fi
