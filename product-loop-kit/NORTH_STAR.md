...
- 2026-07-10 · execution moved to Claude Code cloud ROUTINES (claude.ai/code/routines — human directive: subscription-billed, no API key; supersedes both Mac crons, which failed 3 ways, and the briefly-drafted GitHub Actions workflow, deleted). Heartbeat routine (thinking tier) + build routine (sonnet), each run clones the repo, verifier staged from product-loop-kit/agents/ (also un-gitignored at .claude/agents/verifier.md), unrestricted branch push enabled → ships to main. Mac crons retired. Cowork = oversight/judging/reports.
- 2026-07-10 · .gitignore note: docs/outcome/ and scripts/dashboard-server.js are DELIBERATELY local-only (not missing) — loops and cloud clones must never depend on them
