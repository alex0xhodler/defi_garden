...
|---|---|---|---|
| **Baseline** (no fix) | `{x:365.5,y:648.8,w:549.0,h:224.2}` | `{x:0,y:1331,w:1280,h:69}` (fixed, pinned to viewport bottom) | **0** (nothing below the fold at this height) |
...
```
  ...
  ✓ item 224: 1280x780 at rest (scrollY=0) — zero .empty-state* occlusion by .app-footer
  ✓ item 224: 768x780 at rest (scrollY=0) — zero .empty-state* occlusion by .app-footer
  ✓ item 224: 360x780 at rest (scrollY=0) — zero .empty-state* occlusion by .app-footer
  ✓ item 224: 1280x780 true bottom of scroll — arrival asserted, zero .empty-state* occlusion
  ✓ item 224: 768x780 true bottom of scroll — arrival asserted, zero .empty-state* occlusion
  ✓ item 224: 360x780 true bottom of scroll — arrival asserted, zero .empty-state* occlusion
  ✓ item 224: analytics search-home (no .results-section) keeps .app-footer fixed + .app padding-bottom:80px
...
entirely and roll back to the pre-224 committed file (confirmed this the hard
...
| test | result |
|---|---|
| `test_dead_pool.js` | 12/12 passed (5 pre-existing spec-072 criteria + 7 new item-224 criteria) |
...
- Did not touch the `occlusion` lens's 15% detection rate — that is item 225.
