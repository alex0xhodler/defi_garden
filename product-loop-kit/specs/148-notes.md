...
## Rebase onto current `main` — 2026-07-29 (human directive: "merge")

PR #306 could not be merged as-authored: GitHub returned `405 Pull Request has merge conflicts`. Its base
was `main`@`33cc56ae` (07-26) and `main` had taken 13 commits since, four of them daily CI regens of the
same 5,596-file SEO surface this branch rewrites.

**The branch was NOT merged wholesale, and that was the right call — merging it would have silently
reverted two shipped items:**

1. **Item 173** (`cc243611b`, merged today, live on prod): `main`'s `generate-token-pages.js:673` now reads
   `` `${SITE_URL}/?token=${...}&minTvl=${MIN_POOL_TVL}` ``. The 07-26 branch predates it and carries the
   old line without `&minTvl=`. A wholesale merge re-breaks 1,701 token-page CTAs the same day they were
   fixed.
2. **Item 170** (`0bce2cb43`): `main`'s `package.json` runs `node run-tests.js` with `plain`/`browser`
   lanes and a `test:serial` chain of 104 files. The branch's `package.json` is the pre-170 flat chain of
   ~90 — merging it would have deleted `test_seo_cta_targets.js`, `test_llms_rails.js`,
   `test_audit_*.js` and the whole lane runner from the gate.

**What was actually applied** — the predicate only, transplanted onto `main`'s current files:
- `generate-token-pages.js` — `PURE_NUMERIC_REGEX` + `DATE_FRAGMENT_REGEX` consts and the two added
  guards inside `isValidToken()`. `appUrl` (173's line) untouched.
- `generate-sitemap.js` — the same two rules inline in its `isValidToken()`, plus the export so the
  predicate is testable. Byte-equivalent logic to the token-pages mirror.
- `test_token_slug_validity.js` — taken verbatim from the branch (66 assertions).
- `package.json` — **one line**: `node test_token_slug_validity.js` inserted after `test_token_pages.js`
  in `test:serial`, which is what `run-tests.js` parses to build its file list (the branch's own
  registration mechanism no longer exists post-170).

**Non-vacuity re-proved on this checkout, not inherited from the branch:** with the transplant stashed,
`node test_token_slug_validity.js` exits **1** against `origin/main`'s predicate; with it applied, **66
assertions pass, exit 0**.

**Regen delta (playbook `seo-surface-regen-delta.md`), measured against `origin/main`:**

| | count |
|---|---|
| token pages before (`main`) | 2,132 |
| token pages after | 2,118 |
| **dropped — the 148 target set** | **7** — `00 01 17dec2026 20261231 2027 67 8oct2026` |
| dropped — ordinary daily TVL churn | 7 — `bit dgld kvcm matter qi ripe xrp20` |
| added — ordinary daily TVL churn | 2 — `jyai loop` |

Every one of the 7 target drops is pure-numeric or date-shaped. The 7 churn drops and 2 churn adds are
real tickers crossing the $100K floor between `main`'s last CI bake and this regen — the same thing the
daily CI commit does with no code change (`3828f9e63` precedent). `ls tokens/ | grep -E
'^([0-9]+|[0-9]{1,2}(jan|…|dec)[0-9]{2,4})\.html$'` → **empty**.

Digit-leading real tickers still pass, as the branch's positive-control list requires: `1W 4W 13W 0X0
3CRV 1INCH 40AVAX 50EIGEN 2Z 4CHAN` all still have pages.

**Ship mechanics:** merged by rebase-and-land on `main` rather than by merging #306's head, so #306 must be
closed as superseded rather than merged — its content is on `main`, its stale generated tree is not.

### Browser-lane baseline — 7 pre-existing failures, PROVED not inherited by assumption

`node run-tests.js --lane=browser` on this checkout reports 7 FAILs. Per
`playbooks/pre-existing-red-triage.md`, they were baselined rather than assumed:
an isolated `git worktree` was created at `origin/main`, `node_modules` symlinked, and the same 7 files
re-run there → **`TOTAL pass=0 fail=7`**. Identical set, identical failures. None is caused by this diff.

Failing on `main` today: `test_snapshot_first.js`, `test_analytics_fires.js`,
`test_spotlight_attribution.js`, `test_waitlist_seo_entry.js`, `test_waitlist_funnel.js`,
`test_waitlist_pitch.js`, `test_waitlist_microcopy.js`.

Sample assertion (`test_waitlist_microcopy.js`), which shows the class — the rendered micro-disclaimer no
longer matches what the test expects, in BOTH languages:
```
expected: Free to join • Card doesn't exist yet • We'll email you when it does
got:      No wallet needed to explore
expected: 무료 가입 • 카드는 아직 없어요 • 준비되면 이메일로 알려드려요
got:      지갑 없이도 둘러볼 수 있어요
```
That is a copy change that landed without its tests being updated — the tests are red, not the product
necessarily wrong. Four of the seven are waitlist-funnel tests, i.e. the card funnel that NORTH_STAR names
as the downstream input metric. **Ticketed separately as item 176** — out of scope here, and deliberately
NOT fixed in this commit (drive-by fixes are a verifier FAIL).
