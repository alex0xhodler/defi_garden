...
function App() {
  ...
  const updateUrl = (token, chain, poolTypes, protocols, minTvl, minApy) => {
    ...
    const currentParams = typeof window !== 'undefined' && window.location ? new URLSearchParams(window.location.search) : new URLSearchParams();
    ...
    const currentUrl = typeof window !== 'undefined' && window.location ? (window.location.pathname + window.location.search) : '';
      ...
      try {
        ...
        if (snapRes.ok) {
          ...
          if (snap && Array.isArray(snap.pools)) {
            ...
            snap.pools.forEach(p => { if (p && p.pool) snapMap.set(p.pool, p); });
            (data.data || []).forEach(lp => {
              const sp = snapMap.get(lp.pool);
              if (sp) {
      ...
      React.createElement('div', { className: 'pool-header-new' },
        React.createElement('div', { className: 'pool-name-group' },
          ...
          React.createElement('div', { className: 'pool-left-section' },
            React.createElement('div', { className: 'pool-symbol-line' },
