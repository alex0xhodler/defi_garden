...
// Natural Language Parsing Function — extracted to search-parser.js (spec 017)
// so app.js and the test suite share one implementation. Loaded via a plain
// <script> tag in home.html before app.js; exposes the global
// `parseNaturalLanguageQuery` used below.
...
const getFriendlyProtocolName = (protocolName) => {
  ...
  const protocolNameMap = {
    ...
    // Kamino variants (lend + liquidity are separate DefiLlama projects;
    // unify under one friendly name so "kamino" matches both, same as
    // Aave/Uniswap version unification above)
    'kamino-lend': 'Kamino',
