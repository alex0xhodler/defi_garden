...
const getPoolType = (pool) => getPoolTypeShared(pool);
      ...
      if (!token && tokenCandidateText) {
        for (const t of allTokens) {
          ...
          if (t.length >= 3 && tokenCandidateText.toLowerCase().includes(tokenLower)) {
            ...
            const chainNames = ['eth', 'ethereum', 'polygon', 'matic', 'arb', 'arbitrum', 'op', 'optimism', 'bnb', 'bsc', 'binance', 'avax', 'avalanche', 'sol', 'solana', 'ftm', 'fantom', 'zksync', 'base', 'linea', 'celo', 'gnosis', 'moonbeam', 'cronos'];
...
const POOL_ARTIFACT_UUID_RE = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
...
function App() {
  ...
  const [bakedProtocolUrls, setBakedProtocolUrls] = useState({}); // spec 182 — CI-baked data/protocol-urls.json tier (between dynamic and static PROTOCOL_URLS)
  const [protocolUrlsSettled, setProtocolUrlsSettled] = useState(false); // spec 182 — true once the baked-artifact fetch has resolved (success OR failure/404)
  ...
  const [pendingUrlDirectPool, setPendingUrlDirectPool] = useState(null); // spec 182 Territory T4 — pool parked here for the url_direct pool_view emit, deferred until protocolUrlsSettled
  ...
  useEffect(() => {
    ...
    if (urlParams.chain && !urlParams.token) {
      ...
      setShowFilters(true);
      setShowAutocomplete(false);
      const primaryTitle = (urlParams.protocols && urlParams.protocols[0]) || (urlParams.poolTypes && urlParams.poolTypes[0]) || 'DeFi';
  ...
  useEffect(() => {
    ...
    const tryLoadSnapshot = async (startTime, requiredPoolId) => {
      try {
        ...
        if (!metaRes.ok) return { applied: false };
        const meta = await metaRes.json();
        if (!meta || meta.schemaVersion !== 1) return { applied: false };
        const age = Date.now() - new Date(meta.generatedAt).getTime();
        if (!(age >= 0) || age > SNAPSHOT_MAX_AGE_MS) return { applied: false }; // stale/invalid date → live
        const snapRes = await fetch('/data/pools-snapshot.json');
        if (!snapRes.ok) return { applied: false };
        const snap = await snapRes.json();
        if (!snap || snap.schemaVersion !== 1 || !Array.isArray(snap.pools) || snap.pools.length === 0) return { applied: false };
        if (requiredPoolId) {
          ...
          if (!hasRequiredPool) return { applied: false, poolAbsent: true, snapshotPools: snap.pools };
        }
    ...
    const tryLoadPoolArtifact = async (startTime, requiredPoolId, snapshotPools) => {
      try {
        const artRes = await fetch(`/pools/${encodeURIComponent(requiredPoolId)}.json`);
        ...
        const age = Date.now() - new Date(art.generatedAt).getTime();
        if (!(age >= 0) || age > SNAPSHOT_MAX_AGE_MS) return false;
        const record = art.pools[0];
        const matches = record && (
          record.pool === requiredPoolId ||
          `${record.project}-${record.symbol}-${record.chain}` === decodeURIComponent(requiredPoolId)
        );
    ...
    const fetchPoolsInBackground = async () => {
      ...
      try {
        ...
        if (snapshotEligible) {
          const snapResult = await tryLoadSnapshot(startTime, urlParams.pool || null);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch('/data/protocol-urls.json');
        if (!res.ok) return; // Fail silently — dynamic/static tiers remain the fallback.
        const json = await res.json();
        if (!json || json.schemaVersion !== 1 || typeof json.urls !== 'object' ||
            json.urls === null || Array.isArray(json.urls)) {
          return;
        }
  ...
  const availableChains = useMemo(() => {
    if (chainMode && pools.length > 0) {
      ...
      pools.forEach(pool => {
        ...
        const hasToken = symbolMatchesToken(pool.symbol, selectedToken);
  ...
  useEffect(() => {
    ...
    const tokenModePopularChains = ['Ethereum', 'Arbitrum', 'Polygon', 'Optimism', 'Base', 'BNB Chain', 'Avalanche', 'Solana', 'Fantom', 'Linea', 'Gnosis', 'Celo', 'Moonbeam', 'Cronos', 'zkSync Era'];
    ...
    let filtered = pools.filter(pool => {
      ...
      const chainMatch = !selectedChain || selectedChain === 'All' ||
        (selectedChain === 'Popular'
          ? tokenModePopularChains.includes(pool.chain)
          : pool.chain === selectedChain);
  ...
  const getProtocolUrl = (pool) => {
    ...
    return dynamicProtocolUrls[key] ||
      ...
      bakedProtocolUrls[key] ||
      bakedProtocolUrls[pool.project] ||
      PROTOCOL_URLS[key] ||
      null;
  };
  ...
  if (currentView === 'pool-detail' && detailPool) {
    return React.createElement('div', { className: 'app pool-detail-view' },
      ...
      React.createElement('div', { className: `app-header-sticky${isScrolled ? ' is-scrolled' : ''}` },
  ...
  return React.createElement('div', {
    ...
    React.createElement('div', { className: 'container' },
      ...
      subParam && (selectedToken || (chainMode && selectedChain) || deadPoolResolved) && React.createElement('div', { className: 'app-sub-intent-banner animate-on-mount' },
        React.createElement('div', { className: 'intent-banner-content' },
          React.createElement('span', { className: 'intent-banner-icon', 'aria-hidden': 'true' }, '🌱'),
          React.createElement('div', { className: 'intent-banner-text' },
            React.createElement('strong', null, language === 'ko' ? `${subParam.toUpperCase()} 카드 예치 볼트 선택 중` : `Selecting Vault for ${subParam.toUpperCase()} Card`),
            React.createElement('span', null, language === 'ko' ? '아래 목록에서 원하는 풀을 선택하면 해당 구독이 자동 연결됩니다.' : 'Click any pool below to connect your subscription.')
          )
        ),
        React.createElement('a', {
      ...
      (selectedToken || (chainMode && selectedChain) || deadPoolResolved) && React.createElement('div', { className: 'results-section animate-on-mount' },
        filteredPools.length > 0 && !deadPoolResolved ? [
          ...
          React.createElement('div', { className: 'results-panel', key: 'panel' },
            React.createElement('div', { className: 'results-header', key: 'header' },
              ...
              React.createElement('div', { className: 'results-controls' },
                ...
                React.createElement('div', { className: 'sort-control' },
                  React.createElement('span', { className: 'sort-label' }, t('sortByLabel')),
            ...
            viewMode === 'list' && React.createElement('div', { className: 'pool-columns', key: 'columns' },
              React.createElement('span', { className: 'col-pool' }, t('resultsColPool')),
              React.createElement('span', { className: 'col-apy' }, t('resultsColApy')),
              React.createElement('span', { className: 'col-tvl' }, t('resultsColTvl')),
              React.createElement('span', { className: 'col-action', 'aria-hidden': 'true' })
            ),
        ...
        ] : (loading && !emptyStateResolved && !deadPoolResolved)
          ...
          ? React.createElement('div', { className: 'empty-state', key: 'loading' },
              React.createElement('div', { className: 'empty-message' }, t('loadingYields')))
