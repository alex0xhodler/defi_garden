...
var parseNaturalLanguageQuery = (query, allTokens = [], allChains = [], allProtocols = []) => {
  ...
  if (allTokens && allTokens.length > 0) {
    ...
    var ptParentMatch = lowerQuery.match(/^pt-?([a-z0-9]+)$/i);
    var ptUnderlying = ptParentMatch && allTokens.find(t => String(t).toLowerCase() === ptParentMatch[1].toLowerCase());
...
var ChainBadgeLogo = React.memo(({
  ...
  var src = `https://icons.llamao.fi/icons/chains/rsz_${encodeURIComponent(String(chain).toLowerCase())}?w=48&h=48`;
...
var CURATED_DRAWER_TOKENS = [{
  ...
  symbol: 'USDe',
  label: 'USDe',
  url: 'https://raw.githubusercontent.com/trustwallet/assets/master/blockchains/ethereum/assets/0x4c9EDD5852cd905f086C759E8383e09bff1E68B3/logo.png'
}, {
  symbol: 'USDG',
  label: 'USDG',
  url: 'https://icons.llamao.fi/icons/protocols/paxos?w=48&h=48'
}, {
  symbol: 'ETH',
  label: 'ETH',
  ...
  symbol: 'BTC',
  label: 'BTC',
  url: 'https://raw.githubusercontent.com/trustwallet/assets/master/blockchains/binance/assets/BTCB-1DE/logo.png'
}, {
  symbol: 'SOL',
  label: 'SOL',
  ...
  symbol: 'sUSDe',
  label: 'sUSDe',
  url: 'https://icons.llamao.fi/icons/protocols/ethena-usde?w=48&h=48'
}, {
  symbol: 'cbETH',
  label: 'cbETH',
  url: 'https://icons.llamao.fi/icons/protocols/coinbase-wrapped-staked-eth?w=48&h=48'
}, {
  symbol: 'DAI',
  label: 'DAI',
...
var getPoolRiskScore = pool => {
  ...
  var mean30d = typeof pool.apyMean30d === 'number' && pool.apyMean30d > 0 ? pool.apyMean30d : totalApy;
  var deviation = Math.abs(totalApy - mean30d);

  // TVL liquidity credibility boost: >=$100M (+1.0), >=$10M (+0.5), >=$1M (+0.25)
  var tvl = pool.tvlUsd || 0;
  var tvlBoost = tvl >= 100000000 ? 1.0 : tvl >= 10000000 ? 0.5 : tvl >= 1000000 ? 0.25 : 0;

  // Stablecoin or organic base APY stability ratio
  var organicRatio = totalApy > 0 ? (pool.apyBase || totalApy) / totalApy : 0.5;
  var rawScore = totalApy / (deviation + 1.2) * organicRatio + tvlBoost;
...
function formatSmartTvl(num) {
  ...
  if (num >= 1000000000) return `$${(num / 1000000000).toLocaleString('en-US', {
    maximumFractionDigits: 1
  })}B+`;
  if (num >= 1000000) return `$${(num / 1000000).toLocaleString('en-US', {
    maximumFractionDigits: 1
  })}M+`;
  if (num >= 1000) return `$${(num / 1000).toLocaleString('en-US', {
    maximumFractionDigits: 1
  })}K+`;
...
function App() {
  ...
  var [minSharpe, setMinSharpe] = useState(0);
  ...
  var [sortDirection, setSortDirection] = useState('desc'); // 'desc' | 'asc'
  var [customTvlInput, setCustomTvlInput] = useState('');
  var [customApyInput, setCustomApyInput] = useState('');
  var [protocolSearchQuery, setProtocolSearchQuery] = useState('');
  var [chainSearchQuery, setChainSearchQuery] = useState('');
  var [popoverCoords, setPopoverCoords] = useState({
    top: 128,
    left: 0,
    width: 320
  });
  ...
  var [mobileDrawerStage, setMobileDrawerStage] = useState('peek');
  var [activeSmartLens, setActiveSmartLens] = useState('all');
  var touchStartYRef = useRef(0);
  var applySmartLens = lensId => {
  ...
  var cycleMobileDrawerStage = () => {
    setMobileDrawerStage(prev => prev === 'peek' ? 'half' : prev === 'half' ? 'full' : 'peek');
  };
  var handleDrawerTouchStart = e => {
    if (e.touches && e.touches[0]) {
      touchStartYRef.current = e.touches[0].clientY;
    }
  };
  var handleDrawerTouchEnd = e => {
    if (e.changedTouches && e.changedTouches[0]) {
      var deltaY = e.changedTouches[0].clientY - touchStartYRef.current;
      if (deltaY < -40) {
  ...
  var updatePopoverPosition = (type, btnId) => {
    var targetId = btnId || type + '-btn';
    var btn = document.getElementById(targetId);
    if (!btn) return;
    var rect = btn.getBoundingClientRect();
    var width = type === 'protocols' || type === 'chains' ? 360 : 320;
    var padding = 12;
    var maxLeft = typeof window !== 'undefined' ? window.innerWidth - width - padding : 0;
    var clampedLeft = Math.max(padding, Math.min(rect.left, maxLeft));
    setPopoverCoords({
      top: rect.bottom + 8,
      left: clampedLeft,
  ...
  useEffect(() => {
    ...
    if (!selectedToken && (selectedChain === 'All' || selectedChain === 'Popular')) {
      ...
      var _filtered = pools.filter(pool => {
        ...
        var sharpeMatch = minSharpe === 0 || getPoolRiskScore(pool) >= minSharpe;
        return chainMatch && poolTypeMatch && protocolMatch && tvlMatch && apyMatch && sharpeMatch && pool.tvlUsd > 0;
      });
      setFilteredPools(sortPoolsList(_filtered, sortBy, sortDirection, userSortedApy, false));
      setCurrentPage(1);
      return;
    }
    ...
    if (chainMode && selectedChain && !selectedToken) {
      ...
      setFilteredPools(sortPoolsList(_filtered2, sortBy, sortDirection, userSortedApy, false));
      setCurrentPage(1);
      return;
    }
  ...
  var handleCustomTvlApply = () => {
    var val = parseSmartNumber(customTvlInput, minTvl);
    handleTvlSelect(val);
    setActiveDropdown(null);
  };
  var handleCustomApyApply = () => {
    var val = parseSmartNumber(customApyInput, minApy);
    handleApySelect(val);
    setActiveDropdown(null);
  };
  var handleSortToggle = type => {
    if (sortBy === type) {
      setSortDirection(prev => prev === 'desc' ? 'asc' : 'desc');
    } else {
      setSortBy(type);
      setSortDirection('desc');
      if (type === 'apy') setUserSortedApy(true);else setUserSortedApy(false);
    }
  };
  ...
  var renderMobileSpatialDrawer = () => {
    ...
    var smartLenses = [{
      id: 'all',
      icon: 'all',
      label: language === 'ko' ? '모든 수익률' : 'All Yields'
    }, {
      id: 'robinhood',
      icon: 'chains',
      label: language === 'ko' ? 'Robinhood 체인' : 'Robinhood Chain'
    }, {
      id: 'pendle_pts',
      icon: 'yieldderiv',
      label: language === 'ko' ? 'Pendle PTs' : 'Pendle PTs'
    }, {
      id: 'stables',
      icon: 'vault',
      label: language === 'ko' ? '안정 스테이블 > 3%' : 'Stables > 3%'
    }, {
      id: 'high_yield',
      icon: 'apy',
      label: language === 'ko' ? '고수익 > 8%' : 'High APY > 8%'
    }, {
      id: 'base_l2',
      icon: 'zap',
      label: language === 'ko' ? 'Base & L2s' : 'Base & L2s'
    }, {
      id: 'lst',
      icon: 'staking',
      label: language === 'ko' ? '유동성 스테이킹' : 'Liquid Staking'
    }, {
      id: 'rwa',
      icon: 'rwa',
      label: language === 'ko' ? '실물자산 (RWA)' : 'Real World Assets'
    }];
    var popularChains = [{
      name: 'All',
      label: language === 'ko' ? '모든 체인' : 'All Chains'
    }, {
      name: 'Ethereum',
      label: 'Ethereum'
    }, {
    ...
    return [React.createElement('div', {
      ...
      className: 'omnibar-header-row'
    }, React.createElement('div', {
      className: 'smart-lens-stream'
    }, smartLenses.map(lens => React.createElement('span', {
      key: lens.id,
    ...
    }, navIcon(lens.icon), React.createElement('span', null, lens.label)))), React.createElement('div', {
      className: 'omnibar-input-box',
      onClick: () => {
        if (mobileDrawerStage === 'peek') setMobileDrawerStage('half');
      }
    }, navIcon('search'), React.createElement('input', {
      type: 'text',
      ...
      onClick: e => {
        e.stopPropagation();
        cycleMobileDrawerStage();
      }
    }, navIcon(mobileDrawerStage === 'full' ? 'chevronDown' : 'chevronUp')))), React.createElement('div', {
      className: 'drawer-body-scroll'
    }, React.createElement('div', {
      className: 'drawer-section'
    }, React.createElement('div', {
      className: 'section-label'
    }, React.createElement('span', null, language === 'ko' ? '토큰 바로가기' : 'Quick Token Focus'), selectedToken && React.createElement('span', {
      style: {
        color: 'var(--ui-accent)',
        fontSize: '10px',
        cursor: 'pointer'
    ...
    }, language === 'ko' ? '초기화' : 'Clear')), React.createElement('div', {
      className: 'quick-token-row'
    }, CURATED_DRAWER_TOKENS.map(tok => React.createElement('span', {
      key: tok.symbol,
    ...
    }, React.createElement(TokenBadgeLogo, {
      token: tok
    }), React.createElement('span', null, tok.label))))), React.createElement('div', {
      className: 'drawer-section'
    }, React.createElement('div', {
      className: 'section-label'
    }, React.createElement('span', null, language === 'ko' ? '체인 생태계' : 'Chain Ecosystem')), React.createElement('div', {
      className: 'chain-matrix-grid'
    }, popularChains.map(ch => React.createElement('div', {
      key: ch.name,
    ...
    }, React.createElement(ChainBadgeLogo, {
      chain: ch.name
    }), React.createElement('span', {
      className: 'chain-card-name'
    }, ch.label))))), React.createElement('div', {
      className: 'drawer-section'
    }, React.createElement('div', {
      className: 'section-label'
    }, React.createElement('span', null, language === 'ko' ? '정렬 기준' : 'Sort Metric')), React.createElement('div', {
      className: 'sort-segment-row'
    }, React.createElement('button', {
    ...
    }, navIcon('apy'), React.createElement('span', null, 'APY')), React.createElement('button', {
      ...
      className: 'section-label',
      key: 'label'
    }, React.createElement('span', null, language === 'ko' ? '샤프 지수 / 위험 조정 기준' : 'Risk-Adjusted Stability (Sharpe Ratio)'), minSharpe > 0 && React.createElement('span', {
      style: {
        color: 'var(--ui-accent)',
        fontSize: '10px',
        cursor: 'pointer'
    ...
    }, language === 'ko' ? '초기화' : 'Reset')), React.createElement('div', {
    ...
    }, React.createElement('span', null, '0.0 (All Risk)'), React.createElement('span', {
      style: {
        color: 'var(--ui-accent)',
        fontWeight: '700'
      }
    }, minSharpe === 0 ? language === 'ko' ? '모든 위험 등급' : 'All Risk Profiles' : minSharpe >= 2.5 ? `≥ ${minSharpe} Sharpe · ${language === 'ko' ? '최상위 안정성' : 'Pristine Stability'}` : minSharpe >= 1.5 ? `≥ ${minSharpe} Sharpe · ${language === 'ko' ? '검증된 트랙레코드' : 'Battle-Tested'}` : `≥ ${minSharpe} Sharpe · ${language === 'ko' ? '안정성 양호' : 'Good Stability'}`), React.createElement('span', null, '3.0+ (Max)')), React.createElement('div', {
      className: 'yield-card-presets',
      key: 'presets'
    }, [{
      val: 0,
      label: language === 'ko' ? '전체' : 'All Risk'
    }, {
      val: 1.0,
      label: '≥ 1.0'
    }, {
    ...
    }, p.label))))] : sortBy === 'apy' ? [React.createElement('div', {
      className: 'section-label',
      key: 'label'
    }, React.createElement('span', null, language === 'ko' ? '최소 수익률 (APY)' : 'Minimum APY Filter'), minApy > 0 && React.createElement('span', {
      style: {
        color: 'var(--ui-accent)',
        fontSize: '10px',
        cursor: 'pointer'
      ...
      className: 'slider-value-display'
    }, React.createElement('span', null, '0% Floor'), React.createElement('span', {
      style: {
        color: 'var(--ui-accent)',
        fontWeight: '700'
      }
    }, minApy > 0 ? `≥ ${minApy}% APY` : language === 'ko' ? '모든 수익률' : 'Any APY'), React.createElement('span', null, '20%+')), React.createElement('div', {
      className: 'yield-card-presets',
      key: 'presets'
    }, [{
      val: 0,
      label: language === 'ko' ? '전체' : 'Any APY'
    }, {
      val: 3,
      label: '3%+'
    }, {
      ...
      className: 'section-label',
      key: 'label'
    }, React.createElement('span', null, language === 'ko' ? '최소 유동성 (TVL Floor)' : 'Minimum TVL Liquidity Floor'), minTvl > 0 && React.createElement('span', {
      style: {
        color: 'var(--ui-accent)',
        fontSize: '10px',
        cursor: 'pointer'
      ...
      onChange: e => {
        var steps = [0, 10000, 100000, 1000000, 10000000];
        setMinTvl(steps[parseInt(e.target.value, 10)] || 0);
      }
    }), React.createElement('div', {
      className: 'slider-value-display'
    }, React.createElement('span', null, '$0'), React.createElement('span', {
      style: {
        color: 'var(--ui-accent)',
        fontWeight: '700'
      }
    }, minTvl === 0 ? language === 'ko' ? '제한 없음' : 'No Floor ($0)' : minTvl >= 10000000 ? language === 'ko' ? '≥ $10M (기관급 유동성)' : '≥ $10M (Institutional)' : minTvl >= 1000000 ? language === 'ko' ? '≥ $1M (깊은 유동성)' : '≥ $1M (Deep Liquidity)' : minTvl >= 100000 ? language === 'ko' ? '≥ $100K (기본 검증)' : '≥ $100K (Verified Base)' : '≥ $10K+'), React.createElement('span', null, '$10M+')), React.createElement('div', {
      className: 'yield-card-presets',
      key: 'presets'
    }, [{
      val: 0,
    ...
    }, p.label))))])), mobileDrawerStage !== 'peek' && React.createElement('button', {
    ...
    }, `${language === 'ko' ? '결과 보기' : 'Apply Filters'} (${filteredPools.length} ${language === 'ko' ? '개 풀' : 'Pools'})`)), mobileDrawerStage !== 'peek' && React.createElement('div', {
  ...
  React.createElement('div', {
    ...
    className: 'app-nav-label'
  }, minTvl > 0 ? formatSmartTvl(minTvl) : t('navFilterTvl'))), React.createElement('button', {
    ...
    className: 'app-nav-label'
  }, minApy > 0 ? formatSmartApy(minApy) : t('navFilterApy'))))),
  // Results count only
  React.createElement('div', {
  ...
  deadPoolResolved && React.createElement('div', {
    className: 'dead-pool-recovery-card'
  }, React.createElement('div', {
    className: 'recovery-search-prompt'
  }, t('deadPoolRecoveryPrompt')), React.createElement('div', {
    className: 'recovery-chips-grid'
  }, [{
    label: 'USDC',
    query: 'USDC',
    type: 'token'
    ...
    query: 'Morpho',
    type: 'protocol'
  }].map(chip => React.createElement('button', {
    key: chip.label,
    type: 'button',
  ...
  deadPoolResolved && deadPoolAlternatives.items.length > 0 && React.createElement('div', {
    ...
    className: 'empty-submessage'
  }, t('deadPoolAltHeading')), React.createElement('div', {
    className: 'pools-grid'
  ...
  activeDropdown && React.createElement('div', {
    ...
    className: 'sota-popover-header'
  }, React.createElement('span', {
    className: 'sota-popover-title'
  }, t('navFilterChains') || 'Chains'), React.createElement('span', {
    className: 'sota-active-badge'
  }, selectedChain || 'All Chains')), React.createElement('div', {
    className: 'sota-search-box'
  }, React.createElement('input', {
    ...
    className: 'sota-items-list sota-chains-list'
  }, React.createElement('button', {
  ...
  }, 'All Chains'), React.createElement('button', {
  ...
  }, 'Popular (15)'), availableChains.filter(c => !chainSearchQuery || c.toLowerCase().includes(chainSearchQuery.toLowerCase())).map(chain => React.createElement('button', {
    key: chain,
  ...
  }, chain))), React.createElement('div', {
    className: 'sota-popover-footer'
  }, React.createElement('span', {
    className: 'sota-footer-meta'
  }, `${availableChains.length} chains`), React.createElement('button', {
  ...
  activeDropdown === 'protocols' && availableProtocols.all.length > 0 && React.createElement('div', {
    ...
    className: 'sota-popover-title'
  }, t('navFilterProtocols') || 'Protocols'), React.createElement('span', {
    className: 'sota-active-badge'
  }, selectedProtocols.length > 0 ? `${selectedProtocols.length} selected` : 'All Protocols')), React.createElement('div', {
    className: 'sota-search-box'
  }, React.createElement('input', {
    className: 'sota-search-input',
    type: 'text',
  ...
  }, 'Popular (15)')), React.createElement('div', {
    className: 'sota-items-list'
  }, availableProtocols.all.filter(p => !protocolSearchQuery || p.friendlyName.toLowerCase().includes(protocolSearchQuery.toLowerCase())).slice(0, 80).map(protocol => {
    var isSelected = selectedProtocols.includes(protocol.friendlyName);
    return React.createElement('button', {
      ...
      className: `sota-checkbox ${isSelected ? 'checked' : ''}`
    }, isSelected ? '✓' : ''), React.createElement('span', {
      className: 'sota-item-name'
    }, protocol.friendlyName), protocol.poolCount > 0 && React.createElement('span', {
      className: 'sota-count-badge'
    }, protocol.poolCount));
  })), React.createElement('div', {
    className: 'sota-popover-footer'
  }, selectedProtocols.length > 0 ? React.createElement('button', {
    className: 'sota-btn-text',
    onClick: () => setSelectedProtocols([])
  }, 'Clear All') : React.createElement('span', {
    className: 'sota-footer-meta'
  }, `${availableProtocols.all.length} protocols`), React.createElement('button', {
    className: 'sota-done-btn sota-btn-primary',
    onClick: () => setActiveDropdown(null)
  }, 'Done'))),
  // TVL dropdown
  activeDropdown === 'tvl' && React.createElement('div', {
    ...
    className: 'sota-popover-title'
  }, t('navFilterTvl') || 'Minimum TVL'), React.createElement('span', {
    className: 'sota-active-badge'
  }, formatSmartTvl(minTvl))), React.createElement('div', {
    className: 'sota-popover-section'
  }, React.createElement('div', {
    className: 'sota-section-subtitle'
  }, 'Quick Presets'), React.createElement('div', {
    className: 'sota-preset-chips'
  }, [{
    value: 0,
    label: 'No Min'
  }, {
  ...
  }, tvl.label)))), React.createElement('div', {
    className: 'sota-popover-section sota-custom-section'
  }, React.createElement('div', {
    className: 'sota-section-subtitle'
  }, 'Custom Min TVL'), React.createElement('div', {
    className: 'sota-custom-input-row'
  }, React.createElement('span', {
    className: 'sota-input-affix'
  }, '$'), React.createElement('input', {
    ...
    className: 'sota-input-helper'
  }, 'Examples: 250k, 500k, 2.5m, 10m'))),
  // APY dropdown
  activeDropdown === 'apy' && React.createElement('div', {
    ...
    className: 'sota-popover-title'
  }, t('navFilterApy') || 'Minimum APY'), React.createElement('span', {
    className: 'sota-active-badge'
  }, formatSmartApy(minApy))), React.createElement('div', {
    className: 'sota-popover-section'
  }, React.createElement('div', {
    className: 'sota-section-subtitle'
  }, 'Quick Presets'), React.createElement('div', {
  ...
  }, apy.label)))), React.createElement('div', {
    className: 'sota-popover-section sota-custom-section'
  }, React.createElement('div', {
    className: 'sota-section-subtitle'
  }, 'Custom Min APY'), React.createElement('div', {
    className: 'sota-custom-input-row'
  }, React.createElement('input', {
    className: 'sota-custom-input',
    type: 'text',
    ...
    className: 'sota-input-helper'
  }, 'Examples: 4.5, 8, 12.5, 25'))), ...renderMobileSpatialDrawer()));
}
