...
- **Depth comes from a hairline border and a background step, never from a shadow.** Exactly one ELEVATION token exists — `--ui-shadow-overlay` — and only for things that genuinely float (dropdowns, popovers, modals, a sticky header once scrolled). Two non-elevation shadow uses remain by design and are not covered by that rule: focus rings, and the literal inset fades that signal horizontal scrollability inside filter dropdowns (`style.css`, `.global-filter-dropdown .filter-pills-grid`/`.filter-chips-container`). **Known debt (225, not yet migrated):** focus indication is split across two token families — the new `--ui-focus-ring` and a legacy teal `--focus-ring` still live on `.btn`, `.seo-hub-links a`, `.reset-filters-btn`, `.filter-pill`. New rules use `--ui-focus-ring`; never weaken either (focus is a trust rail).
