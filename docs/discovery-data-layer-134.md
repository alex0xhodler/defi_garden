...
all below the $10M trust-rail floor, should we (a) serve a lower-floored "discovery" dataset from
...
result** today — it fires only when the user manually drags the TVL slider below $10M
(`app.js:1159`). A `?token=X` landing whose pools are all sub-$10M resolves to an *instant* empty
...
~20×, creates a second sub-floor "source of truth" that strains the $10M trust rail, and — if
...
Three data producers, one FE reader. **Every fast source floors at $10M by trust-rail design.**

| Layer | Floor | Where | Committed? | Served to FE? |
|---|---|---|---|---|
| Static snapshot (059) | **$10M** | `generate-pools-snapshot.js:52` (`DEFAULT_MIN_TVL=10000000`), applied `:76` (`isRailedIn`), `:218` | Yes — `data/pools-snapshot.json` + per-chain/token slices in `data/pools/{chain,token}/` | **Yes** — the hot path |
| CF Worker poller → D1 (108/109) | **$10M** | `src/poller-core.js:19`, applied `:48` (`if (tvl < DEFAULT_MIN_TVL) return`) | No (D1 rows) | **No** — store-only; FE never reads D1 (`wrangler.toml:4-5`, `src/poller.js:11`) |
...
There is **no floor between $1000 and $10M anywhere, and nothing below $1000 is retained anywhere.**
...
Trust-rail note: `DEFAULT_MIN_TVL = $10M` and `APY_SANITY_LIMIT = 1000%` are mirrored verbatim
...
When `?token=X` has no pool ≥ $10M, the snapshot (which only contains ≥$10M pools) has no matching
...
- It injects `noindex` (`app.js:2390-2404`) and suggests alternatives that **re-apply the $10M floor**
  (`getEmptyStateAlternatives`, `app.js:2359`: `pool.tvlUsd >= DEFAULT_MIN_TVL && !isAnomalousApy(pool)`).
- The one path that *would* pull sub-$10M pools — the escape-hatch live refetch (`app.js:1158-1175`)
  — is keyed on `[minTvl]` and early-returns while `minTvl >= DEFAULT_MIN_TVL` (`app.js:1159`). It
  fires only when the user drags the TVL slider below $10M, **not** when a token result is empty.

So a real token with only sub-$10M pools shows "no yields found" even though live DefiLlama has
...
cache), but it is the **only** source of truthful sub-$10M data — by design, since the poller and
snapshot both floor at $10M and must never lower.
...
- **(a2) Precompute + commit per-token discovery slices ($1000-floored), fetched on-demand only
  ...
  showing a dead sub-$10M pool is *worse* than a live fetch (a fresh trust hazard on the exact
  low-quality pools the $10M floor exists to gate), (iii) it re-serves far more DefiLlama data from
...
- **(a3) Lower the poller/snapshot floor for a "discovery slice."** Directly touches a trust rail
  applied at the poller/snapshot ($10M is on NORTH_STAR's NEVER-weaken list). Even a "slice" creates
  a served, sub-$10M source of truth. Reading it FE-side either bloats the committed snapshot (a1)
...
Sub-$10M pools stay on the explicit, on-demand live path, where the FE's existing client-side
...
| Trust-framing | Second sub-$10M source of truth; strains the $10M rail | Rail untouched; sub-floor pools shown via existing live risk-flagging |
...
2. Fire one live refetch by reusing the existing `loadLive` / escape-hatch machinery
   ...
   and re-run the token filter against live pools (which include sub-$10M).
...
4. Show any surfaced sub-$10M pools through the **existing** client-side anomaly/High-risk flags —
   never a relaxed floor. The $10M *default* filter stays; discovery is an explicit "we looked
...
for a token with only sub-$10M pools and assert the grid populates from live instead of dead-ending
...
The $10M floor is a trust rail, not a perf knob. The honest way to surface sub-floor pools is on an
...
- Trust rails never weakened — no served floor is lowered; $10M stays the default filter and the
...
> option b): when a `?token=X` search sourced from the $10M snapshot resolves empty for the token,
> fire one live `yields.llama.fi/pools` refetch (reuse `loadLive`/escape-hatch, `app.js:1088-1100`/
> `1158-1175`), re-filter against live pools (incl. sub-$10M, shown via existing anomaly/High-risk
