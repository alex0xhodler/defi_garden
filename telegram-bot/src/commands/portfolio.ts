  ...
  handler: async (ctx: BotContext) => {
    ...
    try {
      ...
      try {
        ...
        console.log(`📊 Portfolio: Fetching APY for ${activeProtocolCalls.length} active protocols (optimization: avoiding ${8 - activeProtocolCalls.length} unnecessary API calls)`);
