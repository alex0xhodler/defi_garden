...
exports.createHandler = exports.walletHandler = void 0;
    ...
    handler: async (ctx) => {
        try {
            ...
            if (!wallet) {
                ...
                await ctx.reply("❌ You don't have a wallet yet.\n\nYou can create a new wallet or import an existing one:", { reply_markup: keyboard });
                return;
            }
            ...
            if (wallet.autoCreated) {
                // Auto-created wallet - simplified menu without upgrade option
    ...
    handler: async (ctx) => {
        try {
            ...
            if (!(0, encryption_1.verifyEncryptionKey)()) {
                await ctx.reply("❌ Bot encryption key is not properly configured. Please contact the bot administrator.");
                return;
            }
            ...
            if (existingWallet) {
                ...
                await ctx.reply("⚠️ You already have a wallet set up. Creating a new wallet will replace your current one.\n\n" +
            ...
            const wallet = await (0, token_wallet_1.generateWallet)(userId);
            ctx.session.walletAddress = wallet.address;
            const keyboard = new grammy_1.InlineKeyboard().text("🔑 Export Private Key", "export_key");
