    ...
    handler: async (ctx) => {
        try {
            ...
            if (aaveBalanceNum > 0.10)
                actualActivePositions.push({ protocol: 'Aave', value: aaveBalanceNum });
            if (fluidBalanceNum > 0.10)
                actualActivePositions.push({ protocol: 'Fluid', value: fluidBalanceNum });
            if (compoundBalanceNum > 0.10)
                actualActivePositions.push({ protocol: 'Compound', value: compoundBalanceNum });
            if (morphoBalanceNum > 0.10)
                actualActivePositions.push({ protocol: 'Morpho', value: morphoBalanceNum });
            if (sparkBalanceNum > 0.10)
                actualActivePositions.push({ protocol: 'Spark', value: sparkBalanceNum });
            if (seamlessBalanceNum > 0.10)
                actualActivePositions.push({ protocol: 'Seamless', value: seamlessBalanceNum });
            if (moonwellBalanceNum > 0.10)
                actualActivePositions.push({ protocol: 'Moonwell', value: moonwellBalanceNum });
