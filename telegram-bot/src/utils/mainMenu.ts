...
export async function getMainMenuMessage(firstName: string = "there", walletAddress?: string, userId?: string): Promise<string> {
  const { getCachedHighestAPY } = await import('./cached-apy-helper');
  const highestAPY = await getCachedHighestAPY();
  ...
  if (userId && walletAddress) {
    try {
      ...
      if (totalDeployed > 0.01) {
        ...
        try {
          const { getCachedMultipleProtocolAPYs } = await import('./cached-apy-helper');
          ...
          if (aaveBalanceNum > 0.01) activeProtocols.push('AAVE');
          if (fluidBalanceNum > 0.01) activeProtocols.push('FLUID');
          if (compoundBalanceNum > 0.01) activeProtocols.push('COMPOUND');
          if (morphoBalanceNum > 0.01) activeProtocols.push('MORPHO');
          if (sparkBalanceNum > 0.01) activeProtocols.push('SPARK');
          if (seamlessBalanceNum > 0.01) activeProtocols.push('SEAMLESS');
          if (moonwellBalanceNum > 0.01) activeProtocols.push('MOONWELL');
          if (morphoRe7BalanceNum > 0.01) activeProtocols.push('MORPHO_RE7');
          
          console.log(`📊 Main menu: Using cached APY for ${activeProtocols.length} active protocols (optimization: avoiding ${8 - activeProtocols.length} unnecessary lookups)`);
          ...
          console.warn("Failed to fetch cached APY for main menu, using fallback rates:", error);
        }
      ...
      if (error?.status === 429 || error?.message?.includes('limit exceeded')) {
        ...
        try {
          const { getCachedHighestAPY } = await import('./cached-apy-helper');
          fallbackAPY = await getCachedHighestAPY();
        } catch {
