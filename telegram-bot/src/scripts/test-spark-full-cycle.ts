...
import { deployToSpark, withdrawFromSpark, getSparkBalance } from '../services/spark-defi';
import { 
 ...
 */
async function testSparkFullCycle(): Promise<void> {
  ...
  try {
    ...
    if (parseFloat(initialUsdcBalance.formatted) < minimumRequired) {
      throw new Error(
        `Insufficient USDC balance. Have: ${initialUsdcBalance.formatted} USDC, Need: ${minimumRequired} USDC.\n` +
    ...
    const depositResult = await deployToSpark(mockUserId, config.amount.toString(), smartAccount);
    ...
    const postDepositPosition = await getSparkBalance(smartWalletAddress);
    ...
    const partialExitResult = await withdrawFromSpark(
      mockUserId, 
      partialExitShares.toString(), 
      smartAccount
    );
    ...
    const postPartialExitPosition = await getSparkBalance(smartWalletAddress);
    const postPartialExitUsdcBalance = await checkUSDCBalance(smartWalletAddress);
    ...
    const fullExitResult = await withdrawFromSpark(
      mockUserId, 
