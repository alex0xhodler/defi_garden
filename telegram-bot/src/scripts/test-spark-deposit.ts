...
import { deployToSpark, getSparkBalance } from '../services/spark-defi';
import { 
 ...
 */
async function testSparkDeposit(): Promise<void> {
  ...
  try {
    ...
    const usdcBalance = await checkUSDCBalance(smartWalletAddress);
    ...
    if (parseFloat(usdcBalance.formatted) < minimumRequired) {
      throw new Error(
        `Insufficient USDC balance. Have: ${usdcBalance.formatted} USDC, Need: ${minimumRequired} USDC.\n` +
    ...
    const initialPosition = await getSparkBalance(smartWalletAddress);
    ...
    const depositResult = await deployToSpark(
      mockUserId,
    ...
    const finalPosition = await getSparkBalance(smartWalletAddress);
