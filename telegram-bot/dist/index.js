...
const index_tokens_1 = __importStar(require("./src/commands/index-tokens"));
...
bot.command(index_tokens_1.default.command, index_tokens_1.default.handler);
...
bot.api.setMyCommands([
    ...
    { command: index_tokens_1.default.command, description: index_tokens_1.default.description },
...
async function handleRetryPendingTransaction(ctx) {
    try {
        ...
        if (pending.service === 'index-tokens' && pending.deployFn === 'buyIndexToken') {
            ...
            const { buyIndexToken } = await Promise.resolve().then(() => __importStar(require("./src/services/index-tokens/index-core")));
            const { getIndexTokenById } = await Promise.resolve().then(() => __importStar(require("./src/lib/database")));
            ...
            if (result.success) {
                ...
                await ctx.reply(`✅ **Index Token Purchase Successful!**\n\n` +
                ...
                await ctx.reply(`❌ **Index Token Purchase Failed**\n\n` +
                    `**Error**: ${result.error}\n\n` +
...
async function handleIndexAdjustConfirm(ctx) {
    try {
...
async function handleIndexAdjustChange(ctx) {
    try {
...
async function handleIndexAdjustCancel(ctx) {
    try {
        // Clear all session data
        ctx.session.tempData = {};
        ctx.session.currentAction = undefined;
        await ctx.reply("❌ **Index Token Purchase Cancelled**\n\n" +
...
bot.on("callback_query:data", async (ctx) => {
    ...
    if (callbackData === "withdraw_aave_max" || callbackData === "withdraw_fluid_max" || callbackData === "withdraw_compound_max" || callbackData === "withdraw_morpho_max" || callbackData === "withdraw_spark_max" || callbackData === "withdraw_seamless_max" || callbackData === "withdraw_moonwell_max" || callbackData === "withdraw_morpho-re7_max" ||
        callbackData === "withdraw_fluid_menu" || callbackData === "withdraw_aave_menu" || callbackData === "withdraw_compound_menu" || callbackData === "withdraw_morpho_menu" || callbackData === "withdraw_spark_menu" || callbackData === "withdraw_seamless_menu" || callbackData === "withdraw_moonwell_menu" || callbackData === "withdraw_morpho-re7_menu" ||
        callbackData === "withdraw_fluid_custom" || callbackData === "withdraw_aave_custom" || callbackData === "withdraw_compound_custom" || callbackData === "withdraw_morpho_custom" || callbackData === "withdraw_spark_custom" || callbackData === "withdraw_seamless_custom" || callbackData === "withdraw_moonwell_custom" || callbackData === "withdraw_morpho-re7_custom" ||
    ...
    // Main menu callbacks
        ...
        await index_tokens_1.default.handler(ctx);
        ...
        await (0, index_tokens_1.handleIndexCategorySelection)(ctx, category);
    }
        ...
        if (indexTokenId === "open_coming_soon") {
            await ctx.answerCallbackQuery();
            await ctx.reply("🌍 **OPEN - Open Stablecoin Index**\n\n" +
        ...
        await (0, index_tokens_1.handleIndexTokenSelection)(ctx, indexTokenId);
    }
    else if (callbackData.startsWith("sell_index_token_")) {
        const indexTokenId = callbackData.replace("sell_index_token_", "");
        await (0, index_tokens_1.handleSellIndexToken)(ctx, indexTokenId);
    }
        ...
        await (0, index_tokens_1.handleSellIndexAmount)(ctx, indexTokenId, percentage);
    }
    else if (callbackData.startsWith("sell_index_custom_")) {
        const indexTokenId = callbackData.replace("sell_index_custom_", "");
        await (0, index_tokens_1.handleSellIndexCustomAmount)(ctx, indexTokenId);
    }
    else if (callbackData.startsWith("index_amount_")) {
        const amount = callbackData.replace("index_amount_", "");
        await (0, index_tokens_1.handleIndexQuickAmount)(ctx, amount);
    }
    else if (callbackData === "index_confirm_yes") {
        await (0, index_tokens_1.handleIndexPurchaseConfirmation)(ctx, true);
    }
    else if (callbackData === "index_confirm_no") {
        await (0, index_tokens_1.handleIndexPurchaseConfirmation)(ctx, false);
    }
    else if (callbackData === "index_sell_confirm_yes") {
        await (0, index_tokens_1.handleIndexSellConfirmation)(ctx, true);
    }
    else if (callbackData === "index_sell_confirm_no") {
        await (0, index_tokens_1.handleIndexSellConfirmation)(ctx, false);
    }
    else if (callbackData === "view_index_positions") {
        await (0, index_tokens_1.handleViewIndexPositions)(ctx);
    }
    else if (callbackData === "sell_index_positions") {
        await (0, index_tokens_1.handleSellIndexPositions)(ctx);
    }
    else if (callbackData === "index_retry_after_transfer") {
        const { handleIndexRetryAfterTransfer } = await Promise.resolve().then(() => __importStar(require("./src/commands/index-tokens")));
        await handleIndexRetryAfterTransfer(ctx);
    }
    else if (callbackData === "index_cancel_transfer") {
        const { handleIndexCancelTransfer } = await Promise.resolve().then(() => __importStar(require("./src/commands/index-tokens")));
        ...
        const { handleSelectAddress } = await Promise.resolve().then(() => __importStar(require("./src/commands/index-tokens")));
        ...
        try {
            ...
            if (balanceNum > 0.01) {
                ...
                const { editMessageWithRealtimeAPY } = await Promise.resolve().then(() => __importStar(require("./src/utils/realtime-apy-updater")));
    ...
    // Harvest callbacks
        ...
        ctx.session.tempData = {};
        await ctx.editMessageText("💰 Collection cancelled. Your earnings remain in the accounts earning interest.", {
            reply_markup: new grammy_1.InlineKeyboard()
                .text("📊 View Portfolio", "view_portfolio")
...
bot.on("message:text", async (ctx) => {
    ...
    switch (ctx.session.currentAction) {
        ...
        case "zap_amount":
            ...
            break;
        case "index_amount":
            await (0, index_tokens_1.handleIndexAmountInput)(ctx);
            break;
        case "index_sell_amount":
            await (0, index_tokens_1.handleIndexSellAmountInput)(ctx);
            break;
