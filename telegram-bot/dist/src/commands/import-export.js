...
exports.handleFundMigration = handleFundMigration;
...
exports.exportHandler = {
    ...
    handler: async (ctx) => {
        try {
            ...
            const { hasCoinbaseSmartWallet, checkAllUSDCBalances } = await Promise.resolve().then(() => __importStar(require("../lib/coinbase-wallet")));
            const hasSmartWallet = hasCoinbaseSmartWallet(userId);
            // Get database wallet info for debugging
            const { getWalletByUserId: getDbWallet } = await Promise.resolve().then(() => __importStar(require("../lib/database")));
            ...
            const { getWallet: getTokenWallet, getAaveBalance, getFluidBalance, getCompoundBalance } = await Promise.resolve().then(() => __importStar(require("../lib/token-wallet")));
            ...
            if (aaveBalanceNum > 0.10)
                actualActivePositions.push({ protocol: 'Aave', value: aaveBalanceNum, type: 'yield' });
            if (fluidBalanceNum > 0.10)
                actualActivePositions.push({ protocol: 'Fluid', value: fluidBalanceNum, type: 'yield' });
            if (compoundBalanceNum > 0.10)
                actualActivePositions.push({ protocol: 'Compound', value: compoundBalanceNum, type: 'yield' });
            if (morphoBalanceNum > 0.10)
                actualActivePositions.push({ protocol: 'Morpho', value: morphoBalanceNum, type: 'yield' });
            if (sparkBalanceNum > 0.10)
                actualActivePositions.push({ protocol: 'Spark', value: sparkBalanceNum, type: 'yield' });
            if (seamlessBalanceNum > 0.10)
                actualActivePositions.push({ protocol: 'Seamless', value: seamlessBalanceNum, type: 'yield' });
            if (moonwellBalanceNum > 0.10)
                actualActivePositions.push({ protocol: 'Moonwell', value: moonwellBalanceNum, type: 'yield' });
            ...
            if (balances) {
                ...
                if (smartWalletBalance > 0.01 || significantPositions.length > 0) {
                    ...
                    await ctx.reply(`🔒 *Export Blocked*\n\n` +
                ...
                if (hasSmartWallet) {
                    ...
                    await ctx.reply(`🔒 *PRIVATE KEY EXPORT BLOCKED*\n\n` +
...
async function handleExportConfirmation(ctx, confirmed) {
    try {
        ...
        if (hasCoinbaseSmartWallet(userId)) {
            ...
            const { decrypt } = await Promise.resolve().then(() => __importStar(require("../lib/encryption")));
            ...
            privateKey = (0, token_wallet_1.getPrivateKey)(wallet);
...
async function handleFundMigration(ctx, migrate = true // Always true since migration is mandatory
) {
    try {
        const userId = ctx.session.userId;
        if (!userId) {
        ...
        try {
            ...
            const { checkAllUSDCBalances, transferUsdcGasless } = await Promise.resolve().then(() => __importStar(require("../lib/coinbase-wallet")));
            ...
            if (result.success) {
                await ctx.reply(`✅ *Transfer completed!*\n\n` +
                ...
                if (remainingBalance > 0.01) {
                    ...
                    await ctx.reply(`⚠️ *EXPORT STILL BLOCKED*\n\n` +
                ...
                await ctx.reply(`🔓 *Export Unlocked!*\n\n` +
                ...
                await ctx.reply(`❌ *FUND TRANSFER FAILED*\n\n` +
                    `Error: ${result.error}\n\n` +
            ...
            const keyboard = new grammy_1.InlineKeyboard()
                ...
                .row()
                .text("❓ Get Help", "transfer_help");
            await ctx.reply(`❌ *FUND TRANSFER ERROR*\n\n` +
        ...
        await ctx.reply("❌ An error occurred during fund migration. Please try again later.");
        ctx.session.currentAction = undefined;
    }
}
