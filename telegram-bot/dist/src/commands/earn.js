 ...
 */
async function getYieldOpportunities(token = "USDC", riskLevel = 3, minApy = constants_1.DEFAULT_SETTINGS.MIN_APY) {
    try {
    ...
    handler: async (ctx) => {
        try {
            ...
            if (!wallet) {
                ...
                await ctx.reply(`👋 Hey ${firstName}! You need a wallet to start earning.\n\nLet me help you set that up:`, { reply_markup: keyboard });
                return;
            }
...
async function handlePoolSelection(ctx) {
    try {
        ...
        const userMinApy = ctx.session.settings?.minApy || constants_1.DEFAULT_SETTINGS.MIN_APY;
        ...
        for (const pool of suitablePools) { // Show all qualifying protocols
            const riskScore = calculateRiskScore(pool);
            const safetyIcon = (0, risk_icons_1.riskIcon)(riskScore);
...
async function handleAutoEarn(ctx) {
    try {
        ...
        const safetyIcon = (0, risk_icons_1.riskIcon)(riskScore);
        await ctx.reply(`🐙 **Auto managed by inkvest Selected Best Pool**\n\n` +
            ...
            `• **Safety**: ${(0, risk_icons_1.riskLabel)(riskScore)} Risk\n\n` +
