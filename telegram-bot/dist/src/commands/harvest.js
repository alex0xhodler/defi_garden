 ...
 */
async function calculateMorphoYields(walletAddress, positions, realTimeYields) {
    try {
        const { assetsFormatted } = await (0, morpho_defi_1.getMorphoBalance)(walletAddress);
 ...
 */
async function calculateSparkYields(walletAddress, positions, realTimeYields) {
    try {
        const { assetsFormatted } = await (0, spark_defi_1.getSparkBalance)(walletAddress);
        const currentBalance = parseFloat(assetsFormatted);
        // Get real-time APY from DeFiLlama
 ...
 */
async function calculateSeamlessYields(walletAddress, positions, realTimeYields) {
    try {
        const { assetsFormatted } = await (0, seamless_defi_1.getSeamlessBalance)(walletAddress);
        const currentBalance = parseFloat(assetsFormatted);
        // Get real-time APY from DeFiLlama
 ...
 */
async function calculateMoonwellYields(walletAddress, positions, realTimeYields) {
    try {
        const { assetsFormatted } = await (0, moonwell_defi_1.getMoonwellBalance)(walletAddress);
        const currentBalance = parseFloat(assetsFormatted);
        // Get real-time APY from DeFiLlama
 ...
 */
async function calculateMorphoRe7Yields(walletAddress, positions, realTimeYields) {
    try {
        const { assetsFormatted } = await (0, morpho_re7_defi_1.getMorphoRe7Balance)(walletAddress);
        const currentBalance = parseFloat(assetsFormatted);
        // Get real-time APY from DeFiLlama
        ...
        const morphoRe7Positions = positions.filter(pos => pos.protocol.toLowerCase() === 're7' ||
...
const harvestHandler = {
    ...
    handler: async (ctx) => {
        try {
            ...
            const smartWallet = await (0, coinbase_wallet_1.getCoinbaseSmartWallet)(user.userId);
            ...
            if (positions.length === 0) {
                await ctx.reply(`🌾 *No Positions to Harvest*\n\n` +
            ...
            if (!hasAnyClaimableRewards) {
                ...
                await ctx.reply(`💰 *Your Earnings Status*\n\n` +
...
async function handleHarvestConfirmation(ctx, strategy) {
    try {
        ...
        await ctx.reply(`⏳ **Executing Harvest**\n\n` +
            `${strategyMessage}\n\n` +
            `💰 Processing $${totalYield.toFixed(3)} in yields...\n\n` +
            `Protocol breakdown:\n` +
            protocolYields.map((p) => `• ${p.protocol}: $${p.yieldEarned.toFixed(3)}`).join('\n') +
        ...
        try {
            ...
            if (totalYield > 5.0) {
                setTimeout(async () => {
                    await ctx.reply(`💡 **Excellent collection!** You earned $${totalYield.toFixed(3)} in interest. ` +
            ...
            await ctx.reply(`⚠️ **Partial Harvest Completed**\n\n` +
        ...
        await ctx.reply("❌ **Harvest Failed**\n\n" +
