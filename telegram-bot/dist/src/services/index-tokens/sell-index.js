...
exports.sellIndexToken = sellIndexToken;
 ...
 */
async function sellIndexToken(userId, indexTokenId, tokenAmount) {
    try {
        // ========================================================================
        ...
        if (isMaxSell) {
            ...
            const userBalance = await publicClient.readContract({
                address: indexToken.contractAddress,
                abi: erc20Abi,
            ...
            tokenAmountWei = (0, viem_1.parseUnits)(tokenAmount, tokenDecimals);
        ...
        const quoteResult = await (0, odos_router_1.getOdosQuote)(indexToken.contractAddress, // Input: Index token
        index_constants_1.BASE_TOKENS.USDC, // Output: USDC
        tokenAmountWei, walletAddress);
        ...
        try {
            ...
            if (isMaxSell) {
                ...
                currentAllowance = await publicClient.readContract({
                    ...
                    args: [walletAddress, index_constants_1.INDEX_CONTRACTS.ODOS_ROUTER_V3]
                });
            }
            else {
            ...
            if (currentAllowance < bufferAmount) {
                ...
                if (currentAllowance > 0n) {
                    ...
                    calls.push({
                        ...
                        to: indexToken.contractAddress,
                        args: [
                            index_constants_1.INDEX_CONTRACTS.ODOS_ROUTER_V3,
                            0n // Reset to 0
                        ]
                    });
                }
            ...
            const userOpHash = await bundlerClient.sendUserOperation({
                ...
                paymasterContext: {
                    erc20: index_constants_1.BASE_TOKENS.USDC // Pay gas with USDC (from sale proceeds)
                }
            });
            ...
            try {
                if (tokensRemainingAfterSell <= 0.000001) {
                    ...
                    (0, database_1.updateIndexPositionValue)(existingPosition.id, 0, // No tokens remaining
                    0, // No current value
                    ...
                    (0, database_1.updateIndexPositionValue)(existingPosition.id, tokensRemainingAfterSell, remainingInvestedValue, // Will be updated by price feeds later
                ...
                (0, database_1.saveIndexTransaction)(txHash, userId, indexTokenId, 'sell', usdcReceived, // USDC received
                amount, // Tokens sold
                pricePerToken, 'success');
            ...
            try {
                ...
                (0, database_1.saveIndexTransaction)(failedTxHash, userId, indexTokenId, 'sell', 0, // No USDC received
                amount, pricePerToken, 'failed');
