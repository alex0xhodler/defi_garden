...
exports.buyIndexToken = buyIndexToken;
exports.getUserIndexPortfolioValue = getUserIndexPortfolioValue;
const viem_1 = require("viem");
const coinbase_wallet_1 = require("../../lib/coinbase-wallet");
const database_1 = require("../../lib/database");
const index_constants_1 = require("../../utils/index-constants");
const odos_router_1 = require("./odos-router");
/**
 ...
 */
async function buyIndexToken(userId, indexTokenId, usdcAmount) {
    try {
        // ========================================================================
        ...
        const indexToken = (0, database_1.getIndexTokenById)(indexTokenId);
        if (!indexToken) {
            return { success: false, error: index_constants_1.INDEX_ERRORS.INVALID_INDEX_TOKEN };
        }
        ...
        if (!smartWallet) {
            ...
            return { success: false, error: index_constants_1.INDEX_ERRORS.NO_SMART_WALLET };
        }
        ...
        // ========================================================================
        const usdcAmountWei = (0, viem_1.parseUnits)(usdcAmount, 6); // USDC has 6 decimals
        ...
        const quoteResult = await (0, odos_router_1.getOdosQuote)(index_constants_1.BASE_TOKENS.USDC, indexToken.contractAddress, usdcAmountWei, walletAddress);
        ...
        if (quote.priceImpact > 10.0) {
            ...
            return {
                success: false,
                error: `${index_constants_1.INDEX_ERRORS.SLIPPAGE_TOO_HIGH} (${quote.priceImpact.toFixed(2)}%)`
            };
        }
        // ========================================================================
        ...
        const bundlerClient = await (0, coinbase_wallet_1.createSponsoredBundlerClient)(smartWallet.smartAccount, smartWallet.isDeployed);
        try {
            // Import required modules
            const { cdpPaymasterClient } = await Promise.resolve().then(() => __importStar(require('../../lib/coinbase-wallet')));
            const { erc20Abi } = await Promise.resolve().then(() => __importStar(require('../../utils/abis')));
            ...
            console.log(`   Approval Target: ${index_constants_1.INDEX_CONTRACTS.ODOS_ROUTER_V3}`);
            ...
            const calls = [
                ...
                // This matches your first transaction: approve(spender, value)
                {
                    ...
                    to: index_constants_1.BASE_TOKENS.USDC, // USDC contract
                    args: [
                        index_constants_1.INDEX_CONTRACTS.ODOS_ROUTER_V3, // spender (ODOS Router V3)
                ...
                {
                    to: quote.transaction.to, // ODOS Router V3
                    data: quote.transaction.data, // swapCompact() calldata
            ...
            const userOpHash = await bundlerClient.sendUserOperation({
                ...
                paymasterContext: {
                    erc20: index_constants_1.BASE_TOKENS.USDC // Pay gas with USDC
                }
            });
            ...
            try {
                ...
                const existingPosition = (0, database_1.getIndexPositionByUserAndToken)(userId, indexTokenId);
                if (existingPosition) {
                    ...
                    (0, database_1.updateIndexPositionValue)(existingPosition.id, newTokensOwned, newTotalInvested, // Current value will be updated by price feeds later
                    newAverageBuyPrice);
                    ...
                    (0, database_1.saveIndexPosition)({
                        id: positionId,
                        userId,
                        indexTokenId,
                ...
                (0, database_1.saveIndexTransaction)(txHash, userId, indexTokenId, 'buy', amount, tokensReceived, pricePerToken, 'success');
            ...
            try {
                ...
                (0, database_1.saveIndexTransaction)(failedTxHash, userId, indexTokenId, 'buy', amount, 0, // No tokens received
                pricePerToken, 'failed');
            ...
            let userError = index_constants_1.INDEX_ERRORS.TRANSACTION_FAILED;
            if (txError.message?.includes('insufficient')) {
                userError = index_constants_1.INDEX_ERRORS.INSUFFICIENT_USDC;
            }
            else if (txError.message?.includes('slippage') || txError.message?.includes('price impact')) {
                userError = index_constants_1.INDEX_ERRORS.SLIPPAGE_TOO_HIGH;
            }
                ...
                userError = index_constants_1.INDEX_ERRORS.NETWORK_ERROR;
            }
            return {
                success: false,
                error: userError
        ...
        return {
            success: false,
            error: error.message || index_constants_1.INDEX_ERRORS.TRANSACTION_FAILED
        };
    }
}
/**
 ...
 */
async function getUserIndexPortfolioValue(userId) {
    return {
        totalValue: 0,
        totalInvested: 0,
        totalPnL: 0,
