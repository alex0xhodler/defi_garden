...
exports.transferUSDCToSmartWallet = transferUSDCToSmartWallet;
...
async function transferUSDCToSmartWallet() {
    ...
    try {
        ...
        const { address: smartWalletAddress } = await (0, test_helpers_1.createTestSmartWallet)(config.privateKey);
        ...
        const smartWalletBalance = await (0, test_helpers_1.checkUSDCBalance)(smartWalletAddress);
        console.log(`🏦 Smart Wallet USDC Balance: ${smartWalletBalance.formatted} USDC`);
        // Validate sufficient balance
        const transferAmount = (0, viem_1.parseUnits)(config.amount.toString(), 6);
        ...
        if (receipt.status === 'success') {
            ...
            const finalSmartWalletBalance = await (0, test_helpers_1.checkUSDCBalance)(smartWalletAddress);
