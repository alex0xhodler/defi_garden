...
// Display test banner
function showTestBanner(config, smartWalletAddress, usdcBalance) {
    console.log(`
🧪 MORPHO DEPOSIT TEST
=====================
...
// Main test function
async function testMorphoDeposit(config) {
    const result = {
        success: false,
        startTime: Date.now()
    };
    try {
        ...
        const balance = await (0, test_helpers_1.checkUSDCBalance)(wallet.address);
        ...
        const depositResult = await (0, morpho_defi_1.deployToMorphoPYTH)('test-user', config.amount.toString(), wallet.smartAccount);
        ...
        result.error = (0, test_helpers_1.safeErrorLog)(error, 'Morpho deposit test');
        result.endTime = Date.now();
        return result;
    }
}
