...
exports.testSparkWithdrawal = testSparkWithdrawal;
 ...
 */
async function testSparkWithdrawal() {
    ...
    try {
        ...
        if (isMaxExit) {
            ...
            sharesToWithdraw = config.shares;
            if (sharesAvailable < sharesToWithdraw) {
                throw new Error(`Insufficient SPARKUSDC shares. Have: ${sharesAvailable}, Need: ${sharesToWithdraw}.\n` +
        ...
        const withdrawalResult = await (0, spark_defi_1.withdrawFromSpark)(mockUserId, isMaxExit ? 'max' : sharesToWithdraw.toString(), smartAccount // Pass smartAccount for testing
        );
