...
async function getMainMenuMessage(firstName = "there", walletAddress, userId) {
    const { getConsistentAPY } = await Promise.resolve().then(() => __importStar(require('./consistent-apy')));
    ...
    if (userId && walletAddress) {
        try {
            ...
            if (totalDeployed > 0.01) {
                ...
                const weightedApy = totalValue > 0 ? ((aaveBalanceNum * aaveApy +
                    ...
                    morphoRe7BalanceNum * morphoRe7Apy) / totalValue) : 0;
