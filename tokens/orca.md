...
ORCA shows up in 3 pools here, with rates from 1.83% to 215.13% APY across 1 chains — the rate depends on which protocol and chain you pick, not just the token.

3 of these 3 pools have a trustworthy 30-day average on file, with a median of 39.44% — a useful check against today's number for whether the rate is steady or just having a good day.
...
|---|---|---|---|
| orca-dex | 215.13% | 87.41% | Base rate |
| orca-dex | 1.83% | 0.41% | Base rate |
| orca-dex | 31.74% | 39.44% | Base rate |
...
31.74% APY on orca-dex (Solana), based on live DefiLlama data.
