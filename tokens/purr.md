...
PURR shows up in 3 pools here, with rates from 0.55% to 74.31% APY across 1 chains — the rate depends on which protocol and chain you pick, not just the token.

3 of these 3 pools have a trustworthy 30-day average on file, with a median of 33.40% — a useful check against today's number for whether the rate is steady or just having a good day.
...
|---|---|---|---|
| project-x | 16.80% | 33.40% | Base rate |
| hyperswap-v3 | 0.55% | 4.06% | Base rate |
| ramses-cl-v2 | 74.31% | 122.54% | 48.35% incentives |
...
74.31% APY on ramses-cl-v2 (Hyperliquid L1), based on live DefiLlama data.
