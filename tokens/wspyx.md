...
The highest honest WSPYX yield right now is 0.30% on fluxion-network (Mantle), among 3 pools above the $100K TVL floor. Rates are live from DefiLlama and exclude anomalous (>1000% APY) pools.
...
WSPYX shows up in 2 pools here, with rates from 0.07% to 0.30% APY across 2 chains — the rate depends on which protocol and chain you pick, not just the token.
...
|---|---|---|---|
| uniswap-v3 | 0.07% | 0.22% | Base rate |
| fluxion-network | 0.30% | 0.13% | Base rate |
...
0.30% APY on fluxion-network (Mantle), based on live DefiLlama data.
