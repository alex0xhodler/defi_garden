...
UPUMP shows up in 4 pools here, with rates from 103.43% to 912.21% APY across 1 chains — the rate depends on which protocol and chain you pick, not just the token.

4 of these 4 pools have a trustworthy 30-day average on file, with a median of 123.17% — a useful check against today's number for whether the rate is steady or just having a good day.
...
|---|---|---|---|
| project-x | 103.43% | 99.00% | Base rate |
| project-x | 129.26% | 109.46% | Base rate |
| nest-cl | 108.69% | 136.87% | 100.00% incentives |
| ramses-cl-v2 | 912.21% | 885.88% | 30.79% incentives |
...
912.21% APY on ramses-cl-v2 (Hyperliquid L1), based on live DefiLlama data.
