...
The highest honest JRROYAPYUSD yield right now is 196.65% on royco-v2 (Ethereum), among 3 pools above the $100K TVL floor. Rates are live from DefiLlama and exclude anomalous (>1000% APY) pools.
...
JRROYAPYUSD shows up in 3 pools here, with rates from 8.81% to 196.65% APY across 1 chains — the rate depends on which protocol and chain you pick, not just the token.
...
|---|---|---|---|
| royco-v2 | 196.65% | 256.44% | Base rate |
| pendle | 8.81% | 9.09% | Base rate |
...
196.65% APY on royco-v2 (Ethereum), based on live DefiLlama data.
