# SAVUSD 디파이 수익률

<<<<<<< HEAD
현재 SAVUSD의 가장 높은 정직한 수익률은 Avalanche의 avant-avusd에서 9.47%이며, $100K TVL 기준을 넘는 8개 풀 중 최고예요. 수익률은 DefiLlama의 실시간 데이터이며 이상 수치(APY 1000% 초과) 풀은 제외했어요.

| 프로토콜 | 체인 | APY | TVL |
|---|---|---|---|
| [avant-avusd →](https://www.defi.garden/?pool=c74227a1-e738-4021-bbe1-13363815aecb&src=seo_token) | Avalanche | 9.47% | $96.15M |
=======
현재 SAVUSD의 가장 높은 정직한 수익률은 Avalanche의 avant-avusd에서 9.43%이며, $100K TVL 기준을 넘는 8개 풀 중 최고예요. 수익률은 DefiLlama의 실시간 데이터이며 이상 수치(APY 1000% 초과) 풀은 제외했어요.

| 프로토콜 | 체인 | APY | TVL |
|---|---|---|---|
| [avant-avusd →](https://www.defi.garden/?pool=c74227a1-e738-4021-bbe1-13363815aecb&src=seo_token) | Avalanche | 9.43% | $96M |
>>>>>>> origin/main
| [curve-dex →](https://www.defi.garden/?pool=008de1a7-4b49-48dd-ae72-c0651092ae73&src=seo_token) | Monad | 5.00% | $761.4K |

수익률은 DefiLlama의 실시간 데이터예요. 이 페이지의 풀은 최소 TVL $100K 기준을 충족하고 이상 수치는 제외했어요 — 이는 이 페이지의 게재 기준일 뿐, 안전을 보장하는 것은 아니에요. 투자 조언이 아닌 교육 목적의 정보예요.

## 자주 묻는 질문

### 오늘 SAVUSD의 가장 높은 수익률은 얼마인가요?

<<<<<<< HEAD
DefiLlama 실시간 데이터 기준, Avalanche의 avant-avusd에서 APY 9.47%예요.

### SAVUSD 풀 중 TVL 기준을 통과한 풀은 몇 개인가요?

이 페이지의 $100K TVL 기준을 통과한 실시간 풀은 8개이며, 합산 TVL은 $137.38M예요.
=======
DefiLlama 실시간 데이터 기준, Avalanche의 avant-avusd에서 APY 9.43%예요.

### SAVUSD 풀 중 TVL 기준을 통과한 풀은 몇 개인가요?

이 페이지의 $100K TVL 기준을 통과한 실시간 풀은 8개이며, 합산 TVL은 $137.11M예요.
>>>>>>> origin/main

### 이 수익률은 안전한가요?

이 페이지에 표시된 풀은 최소 TVL $100K 기준을 충족하고 이상 수치(APY 1000% 초과)인 풀을 제외했어요 — 이는 이 페이지의 게재 기준일 뿐, 안전을 보장하는 것은 아니에요. 이는 투자 조언이 아닌 교육 목적의 정보이며, 표시된 수익률과 무관하게 디파이에는 스마트 컨트랙트 및 시장 위험이 따라요.

## 관련 토큰

- [USDC](https://www.defi.garden/ko/tokens/usdc)
- [WETH](https://www.defi.garden/ko/tokens/weth)
- [BUIDL](https://www.defi.garden/ko/tokens/buidl)
- [SUSDE](https://www.defi.garden/ko/tokens/susde)
- [USDE](https://www.defi.garden/ko/tokens/usde)
- [LBTC](https://www.defi.garden/ko/tokens/lbtc)

## 이용 가능한 체인

- [Avalanche](https://www.defi.garden/ko/chains/avalanche)
- [Monad](https://www.defi.garden/ko/chains/monad)

## 마지막 업데이트: August 7, 2026
