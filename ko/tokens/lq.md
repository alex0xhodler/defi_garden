# LQ 디파이 수익률

<<<<<<< HEAD
현재 LQ의 가장 높은 정직한 수익률은 Cardano의 minswap-dex에서 5.33%이며, $100K TVL 기준을 넘는 1개 풀 중 최고예요. 수익률은 DefiLlama의 실시간 데이터이며 이상 수치(APY 1000% 초과) 풀은 제외했어요.

| 프로토콜 | 체인 | APY | TVL |
|---|---|---|---|
| [minswap-dex →](https://www.defi.garden/?pool=1b452a0b-6a59-4517-a484-0b4415eb8bad&src=seo_token) | Cardano | 5.33% | $178.1K |
=======
현재 LQ의 가장 높은 정직한 수익률은 Cardano의 minswap-dex에서 5.46%이며, $100K TVL 기준을 넘는 1개 풀 중 최고예요. 수익률은 DefiLlama의 실시간 데이터이며 이상 수치(APY 1000% 초과) 풀은 제외했어요.

| 프로토콜 | 체인 | APY | TVL |
|---|---|---|---|
| [minswap-dex →](https://www.defi.garden/?pool=1b452a0b-6a59-4517-a484-0b4415eb8bad&src=seo_token) | Cardano | 5.46% | $176.5K |
>>>>>>> origin/main

수익률은 DefiLlama의 실시간 데이터예요. 이 페이지의 풀은 최소 TVL $100K 기준을 충족하고 이상 수치는 제외했어요 — 이는 이 페이지의 게재 기준일 뿐, 안전을 보장하는 것은 아니에요. 투자 조언이 아닌 교육 목적의 정보예요.

## 자주 묻는 질문

### 오늘 LQ의 가장 높은 수익률은 얼마인가요?

<<<<<<< HEAD
DefiLlama 실시간 데이터 기준, Cardano의 minswap-dex에서 APY 5.33%예요.

### LQ 풀 중 TVL 기준을 통과한 풀은 몇 개인가요?

이 페이지의 $100K TVL 기준을 통과한 실시간 풀은 1개이며, 합산 TVL은 $178.1K예요.
=======
DefiLlama 실시간 데이터 기준, Cardano의 minswap-dex에서 APY 5.46%예요.

### LQ 풀 중 TVL 기준을 통과한 풀은 몇 개인가요?

이 페이지의 $100K TVL 기준을 통과한 실시간 풀은 1개이며, 합산 TVL은 $176.5K예요.
>>>>>>> origin/main

### 이 수익률은 안전한가요?

이 페이지에 표시된 풀은 최소 TVL $100K 기준을 충족하고 이상 수치(APY 1000% 초과)인 풀을 제외했어요 — 이는 이 페이지의 게재 기준일 뿐, 안전을 보장하는 것은 아니에요. 이는 투자 조언이 아닌 교육 목적의 정보이며, 표시된 수익률과 무관하게 디파이에는 스마트 컨트랙트 및 시장 위험이 따라요.

## 관련 토큰

- [USDM](https://www.defi.garden/ko/tokens/usdm)
- [ADA](https://www.defi.garden/ko/tokens/ada)
- [IUSD](https://www.defi.garden/ko/tokens/iusd)
- [USDA](https://www.defi.garden/ko/tokens/usda)
- [NIGHT](https://www.defi.garden/ko/tokens/night)
- [MIN](https://www.defi.garden/ko/tokens/min)

## 이용 가능한 체인

- [Cardano](https://www.defi.garden/ko/chains/cardano)

## 마지막 업데이트: August 7, 2026
