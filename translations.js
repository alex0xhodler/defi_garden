...
const translations = {
  en: {
    ...
    emptyStateExplanation: (token, floor) => `No live pools for ${token} clear our ${floor} minimum-TVL safety floor today.`,
    emptyStateExplanationChain: (chain, floor) => `No live pools on ${chain} clear our ${floor} minimum-TVL safety floor today.`,
    poolNotFoundTitle: "This pool is no longer tracked",
    poolNotFoundExplanation: (floor) => `It's dropped out of the live DefiLlama data we rely on — likely delisted or migrated by its protocol. Here are trustworthy alternatives that clear our ${floor} safety floor.`,
    emptyStateAltHeadingChain: (chain, floor) => `Live pools on ${chain} above the ${floor} floor`,
    emptyStateAltHeadingStable: (floor) => `Popular stablecoin pools above the ${floor} floor`,
    ...
    landing: {
      ...
      exampleEth: "ETH staking",
      exampleLending: "Lending on Arbitrum",
  ...
  ko: {
    ...
    emptyStateExplanation: (token, floor) => `현재 ${token}에서 최소 TVL ${floor} 기준을 통과하는 라이브 풀이 없습니다.`,
    emptyStateExplanationChain: (chain, floor) => `현재 ${chain}에는 최소 TVL ${floor} 기준을 통과하는 라이브 풀이 없습니다.`,
    poolNotFoundTitle: "더 이상 추적되지 않는 풀입니다",
    poolNotFoundExplanation: (floor) => `저희가 사용하는 라이브 DefiLlama 데이터에서 이 풀이 사라졌습니다 — 프로토콜에서 상장 폐지되었거나 마이그레이션되었을 가능성이 높습니다. ${floor} 안전 기준을 통과하는 신뢰할 수 있는 대안을 아래에 안내합니다.`,
    emptyStateAltHeadingChain: (chain, floor) => `${floor} 기준을 통과한 ${chain}의 라이브 풀`,
    emptyStateAltHeadingStable: (floor) => `${floor} 기준을 통과한 인기 스테이블코인 풀`,
